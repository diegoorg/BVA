...
class_id = np.array([2, 0, 2, 2, 2, 2, 2, 2])
detections = sv.Detections(xyxy = xyxy, class_id = class_id)
detections.confidence = np.array([     0.8518,     0.51069,     0.73517,     0.82727,     0.68744,     0.81405,      0.8198,     0.73094])
detections.tracker_id = np.array([1, 2, 3, 4, 5, 6, 7, 8])
...
#frame 2
detections.class_id = np.array([0, 2, 2, 2, 2, 2, 2, 2])
observer.upd_observers(detections)
print(player.player_id for player in observer.players)
